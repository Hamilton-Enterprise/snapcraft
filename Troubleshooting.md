...
You don't need a ChatGPT Pro account. Snapcraft uses API keys from your OpenAI developer account. In order to get access to the GPT4 Vision model, log into your OpenAI account and then, follow these instructions:
...
7. Go to Snapcraft and paste it in the Settings dialog under OpenAI key (gear icon). Your key is only stored in your browser. Never stored on our servers.
...
If you've followed these steps, and it still doesn't work, feel free to open a Github issue. This is a self-hosted fork with no hosted version of its own — if you're looking for a ready-to-use hosted product instead, see the original project's hosted app at screenshottocode.com (credited in the README).
